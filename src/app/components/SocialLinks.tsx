...
const SocialLinks = () => {
  return (
    <div className='max-[600px]:w-11/12 max-[600px]:flex-col max-[600px]:gap-2 max-[600px]:mt-10 h-auto md:w-auto flex md:flex-row gap-10 mt-5'>
      <Link href={`https://twitter.com/dougyoshii`}>
        <div className='flex flex-row gap-2 items-center p-1 rounded-md hover:bg-slate-400 hover:cursor-pointer hover:text-black'>
          <FaTwitterSquare className='hover:cursor-pointer' size={32} />
          <label className='hover:cursor-pointer' >Twitter</label>
        </div>
      </Link>
      <Link href={`https://github.com/douglas-henrique`}>
        <div className='flex flex-row gap-2 items-center p-1 rounded-md hover:bg-slate-400 hover:cursor-pointer hover:text-black'>
          <FaGithubSquare className='hover:cursor-pointer' size={32} />
          <label className='hover:cursor-pointer' >GitHub</label>
        </div>
      </Link>
      <Link href={`https://www.youtube.com/channel/UCIMpIRnb3xlmuZ2_rkdUttA`}>
        <div className='flex flex-row gap-2 items-center p-1 rounded-md hover:bg-slate-400 hover:cursor-pointer hover:text-black'>
          <FaYoutubeSquare className='hover:cursor-pointer' size={32} />
